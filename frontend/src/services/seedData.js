...
export async function seedExampleData() {
  try {
    ...
    const vaca1 = await storageService.create('vacas', {
      ...
      status: 'lactando',
      ultimoCio: new Date(Date.now() - 21 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    });
    ...
    const vaca2 = await storageService.create('vacas', {
      ...
      status: 'lactando',
      ultimoCio: new Date(Date.now() - 42 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    });
    ...
    const vaca3 = await storageService.create('vacas', {
      ...
      status: 'prenha',
      ultimoCio: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    });
    ...
    await storageService.create('medicamentos', {
      ...
      data: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    ...
    await storageService.create('medicamentos', {
      vacaId: vaca2,
      nome: 'Vacina anual',
      data: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
      descricao: 'Vacinação de rotina',
      valor: 35.00,
    });

    // ===== Sanitário =====
    const dataProxima = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    await storageService.create('despesas', {
      data: new Date(hoje.split('-')[0] + '-' + mesAtual.split('-')[1] + '-01').toISOString().slice(0, 10),
      categoria: 'Manutenção',
      descricao: 'Conserto de bomba d\\'água',
      valor: 320.00,
    });
